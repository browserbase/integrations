...
providers, ...) are reachable; Browserbase is not on that list yet, so on a
restricted tier (Tier 1/2) the TLS handshake to api.browserbase.com is reset
even if you pin its IPs — the IP-level `networkAllowList` does NOT bypass the
SNI filter. To run this template, EITHER:
  - use a Tier 3/4 Daytona org (full internet egress by default), OR
  - get Browserbase added to Daytona's domain allowlist: open a PR to
    https://github.com/daytonaio/sandbox-network-whitelist adding
    `api.browserbase.com` and the `connect.*.browserbase.com` regions you use
    (wildcards match only one label), or email support@daytona.io.
