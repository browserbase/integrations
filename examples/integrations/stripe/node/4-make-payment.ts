...
(async () => {
  ...
  // log session ID url
  console.log(
    `Session URL: https://www.browserbase.com/sessions/${session.id}`
  );
  ...
  await page?.fill(
    "input[name='bill_to_forename']",
    paymentInfo.cardholder_firstName!
  );
  await page?.fill(
    "input[name='bill_to_surname']",
    paymentInfo.cardholder_lastName!
  );
  await page?.fill(
    "input[name='bill_to_email']",
    paymentInfo.cardholder_email!
  );
  await page?.fill(
    "input[name='bill_to_phone']",
    paymentInfo.cardholder_phone!
  );

  // Fill in the address information
  await page?.fill(
    "input[name='bill_to_address_line1']",
    paymentInfo.cardholder_address.line1!
  );
  await page?.fill(
    "input[name='bill_to_address_city']",
    paymentInfo.cardholder_address.city!
  );
  await page?.fill(
    "input[name='bill_to_address_postal_code']",
    paymentInfo.cardholder_address.postal_code!
  );
  await page?.selectOption(
    "select#bill_to_address_state",
    paymentInfo.cardholder_address.state!
  );
