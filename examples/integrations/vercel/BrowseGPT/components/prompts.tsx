  ...
  <BlurFade key={index} delay={1 + index * 0.05} inView>
    ...
    >
      <div className="flex flex-col h-full w-full p-1 sm:p-4">
        <div className="mb-auto self-start w-4 h-4 sm:w-6 sm:h-6 m-1 sm:m-0">
          {icon}
        </div>
        <p className="text-[10px] sm:text-sm lg:text-lg text-start mt-auto">
          {text}
        </p>
      </div>
    </MagicCard>
    ...
    <div className="flex justify-center w-full">
      <div className="grid grid-cols-3 gap-6 sm:gap-7 max-w-[400px] sm:max-w-[600px] lg:max-w-[900px]">
        ...
        <PromptCard
          ...
          onClick={() => {
            onPromptClick(
              "What's the top recent news about on https://news.ycombinator.com/"
            );
          }}
        />
        <PromptCard
