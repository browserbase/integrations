"""
FormFillingNode - Voice agent that fills web forms in real-time using Stagehand
"""
...
class FormFillingNode(ReasoningNode):
    """
    Voice agent that fills actual web forms while conducting conversations.
    - Uses Stagehand to read and fill web forms dynamically
    - Maintains conversation flow while automating browser actions
    ...
    def __init__(
        ...
        You are conducting a voice conversation to help fill out a web form.
        As you collect information, it's being entered into an actual online form in real-time.
        Ask natural questions to gather the required information.
        Use the record_form_field tool to save each piece of information.
    ...
    def _create_questions(self) -> list[FormQuestion]:
        ...
        form_questions = [
            ...
            FormQuestion(
                ...
                question="Which role are you applying for? We have openings for Sales Manager, IT Support, Recruiting, Software Engineer, or Marketing Specialist.",
                field_type="checkbox",
                required=True
            ),
            FormQuestion(
            ...
            FormQuestion(
                ...
                question="What relevant skills and experience do you have that make you a strong candidate for this position?",
                field_type="textarea",
                required=True
            ),
            FormQuestion(
    ...
    async def _submit_form(self):
        ...
        try:
            ...
            # (it should already have it from fill_field calls, but ensure consistency)
            self.stagehand_filler.collected_data.update(self.collected_data)
            
            # Use StagehandFormFiller's submit_form method which now uses collected_data
            ...
            if success:
                ...
                logger.warning("⚠️ Form submission may have failed")
                return False
    ...
    async def process_context(
        ...
        logger.info(f"📊 Current question: {current_question.field_name if current_question else 'None'}")
        ...
        if not context.events:
            logger.info(f"📝 Starting conversation - Agent speaks first")
            initial_greeting = (
        ...
        # If last event was our greeting, and user responded, ask first question
        if len(context.events) == 2 and is_agent_response == False and self.current_question_index == 0:
        ...
        if not current_question and self.current_question_index == 0:
            logger.warning("⚠️ No questions available or not properly initialized")
            return
        ...
        Listen to the user's response and use the record_form_field tool to save it.
