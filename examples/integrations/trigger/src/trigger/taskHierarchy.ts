  ...
  run: async (
    {
      useWaits = true,
      useBatch = false,
    }: { useWaits: boolean; useBatch: boolean },
    { ctx }
  ) => {
...
export const childTask = task({
  ...
  run: async ({
    ...
    if (useWaits) {
      if (useBatch) {
        await grandChildTask.batchTriggerAndWait([
          { payload: { useWaits, useBatch } },
        ]);
      } else {
      ...
      if (useBatch) {
        await grandChildTask.batchTrigger([
          { payload: { useWaits, useBatch } },
        ]);
      } else {
...
export const grandChildTask = task({
  ...
  run: async ({
    ...
    if (useWaits) {
      if (useBatch) {
        await greatGrandChildTask.batchTriggerAndWait([
          { payload: { useWaits, useBatch } },
        ]);
      } else {
      ...
      if (useBatch) {
        await greatGrandChildTask.batchTrigger([
          { payload: { useWaits, useBatch } },
        ]);
      } else {
...
async function logRunHierarchy(
  ...
  const triggerInfo = triggerFunction
    ? ` (triggered by ${triggerFunction})`
    : "";
  const parentInfo = parentTaskIdentifier
    ? ` (parent task: ${parentTaskIdentifier})`
    : "";
  ...
  for (const childRun of runData.relatedRuns.children ?? []) {
    await logRunHierarchy(
      childRun.id,
      runData.taskIdentifier,
      childRun.triggerFunction
    );
  }
}
