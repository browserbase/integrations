...
results. Compose those focused tools for multi-step work. Browser tools share
one Browserbase session per Eve session. Parallel browser calls from the same
Eve workflow step are queued within that step; later steps reconnect through
durable state. Prefer calling tools sequentially, and stop the browser session
